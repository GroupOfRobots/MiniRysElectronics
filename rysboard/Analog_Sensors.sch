EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 5 7
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
Text HLabel 3150 2875 0    50   Input ~ 0
BOARD_T_1
Text HLabel 6150 1275 0    50   Input ~ 0
CHASSIS_T_2
Text HLabel 2425 4375 0    50   Input ~ 0
V_P_20
Text HLabel 3575 4375 0    50   Input ~ 0
V_P_5
Text HLabel 1600 5650 0    50   Input ~ 0
BAT_CS
Text HLabel 6025 4550 0    50   Input ~ 0
V_BAT_C1
Text HLabel 7050 4550 0    50   Input ~ 0
V_BAT_C2
Text HLabel 7850 4550 0    50   Input ~ 0
V_BAT_C3
Wire Wire Line
	3150 2525 3150 2575
$Comp
L power:GND #PWR049
U 1 1 603EF9F9
P 2750 3275
F 0 "#PWR049" H 2750 3025 50  0001 C CNN
F 1 "GND" H 2755 3102 50  0000 C CNN
F 2 "" H 2750 3275 50  0001 C CNN
F 3 "" H 2750 3275 50  0001 C CNN
	1    2750 3275
	-1   0    0    -1  
$EndComp
$Comp
L Device:Thermistor_PTC TH2
U 1 1 603F0105
P 3150 3025
F 0 "TH2" H 3248 3071 50  0000 L CNN
F 1 "Thermistor_PTC" H 3248 2980 50  0000 L CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 3200 2825 50  0001 L CNN
F 3 "~" H 3150 3025 50  0001 C CNN
	1    3150 3025
	1    0    0    -1  
$EndComp
Wire Wire Line
	2750 3175 2750 3275
Wire Wire Line
	3150 3175 2750 3175
$Comp
L power:+5V #PWR051
U 1 1 603F2BD7
P 3875 4075
F 0 "#PWR051" H 3875 3925 50  0001 C CNN
F 1 "+5V" H 3890 4248 50  0000 C CNN
F 2 "" H 3875 4075 50  0001 C CNN
F 3 "" H 3875 4075 50  0001 C CNN
	1    3875 4075
	1    0    0    -1  
$EndComp
Wire Wire Line
	3875 4375 3575 4375
Text GLabel 2050 5650 2    50   Output ~ 0
Bat_Current_Pos
Wire Wire Line
	2050 5650 1600 5650
Text GLabel 6250 4250 1    50   Output ~ 0
V_CELL1
Text GLabel 7050 4250 1    50   Output ~ 0
V_CELL2
Text GLabel 7850 4250 1    50   Output ~ 0
V_CELL3
$Comp
L Device:R R52
U 1 1 60405D8A
P 2775 4225
F 0 "R52" H 2845 4271 50  0000 L CNN
F 1 "9KR0.5%" H 2845 4180 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 2705 4225 50  0001 C CNN
F 3 "~" H 2775 4225 50  0001 C CNN
	1    2775 4225
	1    0    0    -1  
$EndComp
$Comp
L Device:R R53
U 1 1 60406110
P 2775 4525
F 0 "R53" H 2705 4479 50  0000 R CNN
F 1 "1KR0.5%" H 2705 4570 50  0000 R CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 2705 4525 50  0001 C CNN
F 3 "~" H 2775 4525 50  0001 C CNN
	1    2775 4525
	-1   0    0    1   
$EndComp
$Comp
L power:GND #PWR028
U 1 1 6040647E
P 2775 4675
F 0 "#PWR028" H 2775 4425 50  0001 C CNN
F 1 "GND" H 2780 4502 50  0000 C CNN
F 2 "" H 2775 4675 50  0001 C CNN
F 3 "" H 2775 4675 50  0001 C CNN
	1    2775 4675
	-1   0    0    -1  
$EndComp
Wire Wire Line
	2425 4375 2775 4375
Connection ~ 2775 4375
$Comp
L power:GND #PWR052
U 1 1 604093E1
P 3875 4675
F 0 "#PWR052" H 3875 4425 50  0001 C CNN
F 1 "GND" H 3880 4502 50  0000 C CNN
F 2 "" H 3875 4675 50  0001 C CNN
F 3 "" H 3875 4675 50  0001 C CNN
	1    3875 4675
	-1   0    0    -1  
$EndComp
Text Notes 4675 4675 0    50   ~ 0
33v == 3.3v\n\nR=U/I\nI=U/R1+R2\nR1 =90KOhm\nR2 = 10KOhm\n\n9.9 == 3.3V\nR1 = 10KOhm\nR2 =  5KOhm
Text Notes 4625 3725 0    50   ~ 0
TO DO:\nDouble check values\n
$Comp
L Device:R R62
U 1 1 6037F93A
P 6250 4400
F 0 "R62" H 6320 4446 50  0000 L CNN
F 1 "10k1%" H 6320 4355 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 6180 4400 50  0001 C CNN
F 3 "~" H 6250 4400 50  0001 C CNN
	1    6250 4400
	1    0    0    -1  
$EndComp
$Comp
L Device:R R63
U 1 1 6037FE25
P 6250 4700
F 0 "R63" H 6320 4746 50  0000 L CNN
F 1 "20KR" H 6320 4655 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 6180 4700 50  0001 C CNN
F 3 "~" H 6250 4700 50  0001 C CNN
	1    6250 4700
	1    0    0    -1  
$EndComp
$Comp
L Device:R R65
U 1 1 60380130
P 7050 4700
F 0 "R65" H 7120 4746 50  0000 L CNN
F 1 "5KR" H 7120 4655 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 6980 4700 50  0001 C CNN
F 3 "~" H 7050 4700 50  0001 C CNN
	1    7050 4700
	1    0    0    -1  
$EndComp
$Comp
L Device:R R67
U 1 1 603804EB
P 7850 4700
F 0 "R67" H 7920 4746 50  0000 L CNN
F 1 "2.8KR" H 7920 4655 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 7780 4700 50  0001 C CNN
F 3 "~" H 7850 4700 50  0001 C CNN
	1    7850 4700
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR055
U 1 1 60382E1E
P 6250 4850
F 0 "#PWR055" H 6250 4600 50  0001 C CNN
F 1 "GND" H 6255 4677 50  0000 C CNN
F 2 "" H 6250 4850 50  0001 C CNN
F 3 "" H 6250 4850 50  0001 C CNN
	1    6250 4850
	-1   0    0    -1  
$EndComp
$Comp
L power:GND #PWR056
U 1 1 60383282
P 7050 4850
F 0 "#PWR056" H 7050 4600 50  0001 C CNN
F 1 "GND" H 7055 4677 50  0000 C CNN
F 2 "" H 7050 4850 50  0001 C CNN
F 3 "" H 7050 4850 50  0001 C CNN
	1    7050 4850
	-1   0    0    -1  
$EndComp
$Comp
L power:GND #PWR057
U 1 1 603834B0
P 7850 4850
F 0 "#PWR057" H 7850 4600 50  0001 C CNN
F 1 "GND" H 7855 4677 50  0000 C CNN
F 2 "" H 7850 4850 50  0001 C CNN
F 3 "" H 7850 4850 50  0001 C CNN
	1    7850 4850
	-1   0    0    -1  
$EndComp
Text Notes 6250 3650 0    50   ~ 0
0-4.95V\n
Text Notes 6950 3650 0    50   ~ 0
0-9.9V\n
Text Notes 7700 3650 0    50   ~ 0
0-15.086V\n
$Comp
L Device:R R56
U 1 1 603B9257
P 3875 4225
F 0 "R56" H 3945 4271 50  0000 L CNN
F 1 "10k1%" H 3945 4180 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 3805 4225 50  0001 C CNN
F 3 "~" H 3875 4225 50  0001 C CNN
	1    3875 4225
	1    0    0    -1  
$EndComp
$Comp
L Device:R R57
U 1 1 603B976E
P 3875 4525
F 0 "R57" H 3945 4571 50  0000 L CNN
F 1 "5KR" H 3945 4480 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 3805 4525 50  0001 C CNN
F 3 "~" H 3875 4525 50  0001 C CNN
	1    3875 4525
	1    0    0    -1  
$EndComp
Connection ~ 3875 4375
Text GLabel 8625 1800 1    50   Output ~ 0
MCU_3.3V
$Comp
L Device:R R55
U 1 1 603BD637
P 3150 2725
F 0 "R55" H 3220 2771 50  0000 L CNN
F 1 "10k1%" H 3220 2680 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 3080 2725 50  0001 C CNN
F 3 "~" H 3150 2725 50  0001 C CNN
	1    3150 2725
	1    0    0    -1  
$EndComp
Wire Wire Line
	8625 1800 8775 1800
Wire Wire Line
	9225 1800 9225 1900
$Comp
L Device:R R66
U 1 1 603C19CD
P 8775 1950
F 0 "R66" H 8845 1996 50  0000 L CNN
F 1 "100KR" H 8845 1905 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 8705 1950 50  0001 C CNN
F 3 "~" H 8775 1950 50  0001 C CNN
	1    8775 1950
	1    0    0    -1  
$EndComp
Connection ~ 8775 1800
Wire Wire Line
	8775 1800 9225 1800
$Comp
L Device:R R64
U 1 1 603C1DE2
P 8625 2100
F 0 "R64" H 8695 2146 50  0000 L CNN
F 1 "10k1%" H 8695 2055 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 8555 2100 50  0001 C CNN
F 3 "~" H 8625 2100 50  0001 C CNN
	1    8625 2100
	0    1    1    0   
$EndComp
Wire Wire Line
	8775 2100 8925 2100
Connection ~ 8775 2100
Text HLabel 8325 2100 0    50   Input ~ 0
S_E
Wire Wire Line
	8325 2100 8475 2100
Text Notes 8450 1000 0    50   ~ 0
When not used, thermistors do not need to be powered on.
$Comp
L Device:R R61
U 1 1 604C3349
P 6150 1075
F 0 "R61" H 6220 1121 50  0000 L CNN
F 1 "10k1%" H 6220 1030 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 6080 1075 50  0001 C CNN
F 3 "~" H 6150 1075 50  0001 C CNN
	1    6150 1075
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x02 J23
U 1 1 604C3809
P 6250 1575
F 0 "J23" V 6122 1655 50  0000 L CNN
F 1 "CH2_T" V 6213 1655 50  0000 L CNN
F 2 "Connector_PinHeader_1.27mm:PinHeader_1x02_P1.27mm_Vertical" H 6250 1575 50  0001 C CNN
F 3 "~" H 6250 1575 50  0001 C CNN
	1    6250 1575
	0    1    1    0   
$EndComp
$Comp
L Connector_Generic:Conn_01x02 J22
U 1 1 604C3EA4
P 5350 1550
F 0 "J22" V 5222 1630 50  0000 L CNN
F 1 "CH1_T" V 5313 1630 50  0000 L CNN
F 2 "Connector_PinHeader_1.27mm:PinHeader_1x02_P1.27mm_Vertical" H 5350 1550 50  0001 C CNN
F 3 "~" H 5350 1550 50  0001 C CNN
	1    5350 1550
	0    1    1    0   
$EndComp
$Comp
L Device:R R59
U 1 1 604C4170
P 5250 1050
F 0 "R59" H 5320 1096 50  0000 L CNN
F 1 "10k1%" H 5320 1005 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 5180 1050 50  0001 C CNN
F 3 "~" H 5250 1050 50  0001 C CNN
	1    5250 1050
	1    0    0    -1  
$EndComp
Wire Wire Line
	6150 1225 6150 1375
Wire Wire Line
	5250 1200 5250 1350
$Comp
L power:GND #PWR0108
U 1 1 604C7572
P 6250 1375
F 0 "#PWR0108" H 6250 1125 50  0001 C CNN
F 1 "GND" H 6255 1202 50  0000 C CNN
F 2 "" H 6250 1375 50  0001 C CNN
F 3 "" H 6250 1375 50  0001 C CNN
	1    6250 1375
	0    -1   -1   0   
$EndComp
$Comp
L power:GND #PWR0109
U 1 1 604C77C4
P 5350 1350
F 0 "#PWR0109" H 5350 1100 50  0001 C CNN
F 1 "GND" H 5355 1177 50  0000 C CNN
F 2 "" H 5350 1350 50  0001 C CNN
F 3 "" H 5350 1350 50  0001 C CNN
	1    5350 1350
	0    -1   -1   0   
$EndComp
Text GLabel 9850 2300 2    50   Input ~ 0
MCU_ANALOG_SENSOR_V
Wire Wire Line
	9850 2300 9600 2300
Text GLabel 6150 925  2    50   Input ~ 0
MCU_ANALOG_SENSOR_V
Text GLabel 5250 750  2    50   Input ~ 0
MCU_ANALOG_SENSOR_V
Wire Wire Line
	5250 750  5250 900 
Text GLabel 3150 2525 0    50   Input ~ 0
MCU_ANALOG_SENSOR_V
Text HLabel 5250 1275 0    50   Input ~ 0
CHASSIS_T_1
$Comp
L Device:R R70
U 1 1 603B8902
P 7050 4400
F 0 "R70" H 7120 4446 50  0000 L CNN
F 1 "10k1%" H 7120 4355 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 6980 4400 50  0001 C CNN
F 3 "~" H 7050 4400 50  0001 C CNN
	1    7050 4400
	1    0    0    -1  
$EndComp
$Comp
L Device:Thermistor_PTC TH1
U 1 1 6063752B
P 3975 3025
F 0 "TH1" H 4073 3071 50  0000 L CNN
F 1 "Thermistor_PTC" H 4073 2980 50  0000 L CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 4025 2825 50  0001 L CNN
F 3 "~" H 3975 3025 50  0001 C CNN
	1    3975 3025
	1    0    0    -1  
$EndComp
Wire Wire Line
	3975 3175 3150 3175
Connection ~ 3150 3175
$Comp
L Device:R R31
U 1 1 6063841E
P 3975 2700
F 0 "R31" H 4045 2746 50  0000 L CNN
F 1 "10k1%" H 4045 2655 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 3905 2700 50  0001 C CNN
F 3 "~" H 3975 2700 50  0001 C CNN
	1    3975 2700
	1    0    0    -1  
$EndComp
Wire Wire Line
	3150 2525 3975 2525
Wire Wire Line
	3975 2525 3975 2550
Wire Wire Line
	3975 2875 3975 2850
$Comp
L power:+BATT #PWR0115
U 1 1 60625717
P 2775 4075
F 0 "#PWR0115" H 2775 3925 50  0001 C CNN
F 1 "+BATT" H 2790 4248 50  0000 C CNN
F 2 "" H 2775 4075 50  0001 C CNN
F 3 "" H 2775 4075 50  0001 C CNN
	1    2775 4075
	1    0    0    -1  
$EndComp
Text HLabel 3975 2875 0    50   Input ~ 0
BOARD_T_2
$Comp
L Device:C C32
U 1 1 6069F647
P 9600 2450
F 0 "C32" H 9715 2496 50  0000 L CNN
F 1 "100n" H 9715 2405 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric" H 9638 2300 50  0001 C CNN
F 3 "~" H 9600 2450 50  0001 C CNN
	1    9600 2450
	1    0    0    -1  
$EndComp
Connection ~ 9600 2300
Wire Wire Line
	9600 2300 9225 2300
$Comp
L power:GND #PWR0116
U 1 1 6069FD9A
P 9600 2600
F 0 "#PWR0116" H 9600 2350 50  0001 C CNN
F 1 "GND" H 9605 2427 50  0000 C CNN
F 2 "" H 9600 2600 50  0001 C CNN
F 3 "" H 9600 2600 50  0001 C CNN
	1    9600 2600
	-1   0    0    -1  
$EndComp
$Comp
L Transistor_FET:AO3401A Q10
U 1 1 608D8760
P 9125 2100
F 0 "Q10" H 9330 2054 50  0000 L CNN
F 1 "PMV28XPEAR" H 9330 2145 50  0000 L CNN
F 2 "Package_TO_SOT_SMD:SOT-23" H 9325 2025 50  0001 L CIN
F 3 "http://www.aosmd.com/pdfs/datasheet/AO3401A.pdf" H 9125 2100 50  0001 L CNN
	1    9125 2100
	1    0    0    1   
$EndComp
$Comp
L Device:R R71
U 1 1 603B8D2E
P 7850 4400
F 0 "R71" H 7920 4446 50  0000 L CNN
F 1 "10KR" H 7920 4355 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 7780 4400 50  0001 C CNN
F 3 "~" H 7850 4400 50  0001 C CNN
	1    7850 4400
	1    0    0    -1  
$EndComp
Connection ~ 6250 4550
Wire Wire Line
	6025 4550 6250 4550
$EndSCHEMATC
